...
本實驗以「高併發演唱會售票平台」作為案例，先用 C 語言的共享變數售票程式展示 race condition 可能造成的超賣問題，再觀察 Rust 在並行程式設計中的兩種不同工具：

- `std::sync::mpsc`：用於即時訂單處理，展示 message passing、bounded queue 與 backpressure。
- Rayon：用於售後批次分析，展示資料平行處理與 sequential / parallel 效能比較。

需要特別說明的是，C 實驗不是用來和 Rayon 做同類型效能比較，而是作為問題動機與 baseline：它證明未保護的共享可變狀態會破壞售票系統的 business invariant。Rust `mpsc` 實驗則展示如何用 message passing 重新設計即時訂單流程；Rayon 實驗展示售後資料分析如何使用資料平行加速。
...
| sold | 800 | 100 | 100 |
| failed / rejected sold out | 480 | 1180 | 1180 |
| remaining | 0 | 0 | 0 |
| sold by tier | - | - | regular=80, vip=20 |
| remaining by tier | - | - | regular=0, vip=0 |
| invariant 是否成立 | false | true | true |
| oversold | true | false | false |
| producer sends delayed by backpressure | - | - | 1200 |
| cumulative producer send wait | - | - | 11.458594s |
| elapsed | - | - | 0.788177s |

補充：C unsafe 的 race condition 具有非決定性，不同次執行可能出現不同錯誤數字；重點不是固定得到 `sold = 800`，而是 invariant 會被破壞，且 `sold` 可能超過初始庫存 100。
...
- C unsafe 的 `sold = 800`，但初始票數只有 100，表示系統嚴重超賣。
...
| producer sends delayed by backpressure | 1200 | 1248 |
| elapsed | 0.788177s | 1.3359624s |
...
## 5. 實驗二：Rayon 批次分析，1,000,000 筆資料
...
## 6. 實驗三：Rayon 批次分析，2,000,000 筆資料
...
1. C unsafe、C mutex、C queue 形成清楚的工程設計對照。
...
2. Rust `mpsc` 和 C queue 是最直接的對照組。
...
4. Bounded channel 可以呈現 backpressure。
...
7. `mpsc` 與 Rayon 都屬於並行程式設計工具，但解決的問題不同。
...
總結來說，本專案現在形成更完整的對照：C unsafe 展示 race condition 的錯誤後果，C mutex 展示手動加鎖修正，C queue 展示手寫 message passing 架構，Rust `mpsc` 展示標準 channel 與 ownership boundary 的表達方式，Rayon 則展示售後批次資料分析的平行化。這比單純把 C 和 Rayon 放在一起比較更合理，也更貼近軟體工程報告中「問題、修正、架構重設計、批次分析」的完整脈絡。
